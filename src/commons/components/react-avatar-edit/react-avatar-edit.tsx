...
export class Avatar extends Component<any, any> {
  ...
  initCanvas() {
    ...
    const isLeftCorner = (scale?: any) => crop.x() - scaledRadius(scale) < 0;
    const calcLeft = () => crop.radius() + 1;
    const isTopCorner = (scale?: any) => crop.y() - scaledRadius(scale) < 0;
    const calcTop = () => crop.radius() + 1;
    const isRightCorner = (scale?: any) => crop.x() + scaledRadius(scale) > stage.width();
    const calcRight = () => stage.width() - crop.radius() - 1;
    const isBottomCorner = (scale?: any) => crop.y() + scaledRadius(scale) > stage.height();
    const calcBottom = () => stage.height() - crop.radius() - 1;
    const isNotOutOfScale = (scale: any) =>
      !isLeftCorner(scale) && !isRightCorner(scale) && !isBottomCorner(scale) && !isTopCorner(scale);
    const calcScaleRadius = (scale: any) =>
      scaledRadius(scale) >= this.minCropRadius ? scale : crop.radius() - this.minCropRadius;
    const calcResizerX = (x: any) => x + crop.radius() * 0.86;
    const calcResizerY = (y: any) => y - crop.radius() * 0.5;
    const moveResizer = (x: any, y: any) => {
    ...
    const onScaleCallback = (scaleY: any) => {
    ...
    return (
      <div>
        {this.state.showLoader ? (
          ...
          <div style={style}>
            <svg onClick={this.onCloseClick} style={closeBtnStyle} viewBox="0 0 475.2 475.2" width="20px" height="20px">
              <g>
                <path
